...
class NagiosItem():
    ...
    #def setPerfName(self, perf: re.Pattern):
    def setPerfName(self, perf: rePattern):
        m = perf.match(self.podname)
        if m:
            self.perf_prefix = m.group(1)
        else:
            logger.error('failed to get perf pattern for pod {}'.format(self.podname))

    @classmethod
    def _getPerfName(cls, prefix) -> str:
        name = prefix
    ...
    def getPerfName(self) -> str:
        return NagiosItem._getPerfName(self.perf_prefix)
...
class Nagios():
    ...
    def __init__(self, aggregate, perf) -> None:
        ...
        if aggregate == 'none':
            if perf is None:
                perf = r'^([a-z-]+)-.*'
            self.perf = re.compile(perf)
            if self.perf.groups < 1:
                logger.error("Nagios perf regexp pattern (--perf) '{}' does *not* contains any capture group !".format(perf))
        elif perf is None:
            self.perf = ''
            ns = PrometheusLabel.get('container_label_io_kubernetes_pod_namespace')
            if ns and ns.isValueDefined():
                self.perf = '{}-'.format(ns.value)
            pod = PrometheusLabel.get('container_label_io_kubernetes_pod_name')
            rpod = re.compile(r'^([a-z-]+)-.*')
            rmatch = rpod.match(pod.value)
            if rmatch:
                self.perf += rmatch.group(1)
            else:
                logger.error('no perf pattern provided, and was not able to extrapolate one from pod pattern')
                sys.exit(Nagios.NAGIOS_UNKNOWN)
            logger.warning('no perf pattern provided, using {} as perf prefix'.format(self.perf))
        else:
            self.perf = perf
    ...
    def aggregator_sum(self):
        ...
        if len(NagiosItem.instances()) > 0:
            ...
            perf = "|{kp};{va};{wa};{cr};{mi};{ma}".format(
                kp = NagiosItem._getPerfName(self.perf),
                va = round(float(item.value),2),
                wa = self.warning,
                cr = self.critical,
                mi = self.perf_min,
