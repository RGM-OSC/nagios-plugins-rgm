...
def rgm_replication_output(clearpass_host, client_id, client_name, client_password, warning_threshold, critical_threshold):
		try:
				...
				first_dict = {'name':'first','is_master':False,'replication_status':'ENABLED','last_replication_timestamp':'2020-05-21T10:00:00.254+02:00','management_ip':'10.112.11.121'}
				member_list.append(first_dict)
				second_dict = {'name':'second','is_master':False,'replication_status':'ENABLED','last_replication_timestamp':'2020-05-22T09:00:00.254+02:00','management_ip':'10.112.11.122'}
				member_list.append(second_dict)
				third_dict = {'name':'third','is_master':False,'replication_status':'ENABLED','last_replication_timestamp':'2020-05-22T10:00:00.254+02:00','management_ip':'10.112.11.123'}
					...
					if not server['is_master']:
						...
						if server['replication_status'] == 'ENABLED':
							...
							if server['last_replication_timestamp']:
								...
								replication_date = parser.parse(server['last_replication_timestamp'])	
...
if __name__ == '__main__':
        ...
        check the replication status of slaves Clearpass Servers
				...
				python check_clearpass_API_replication.py -H clearpass1 -u user -i clientID -p password
				
				example : python check_clearpass_API_replication.py -H clearpass1 -u rgm -i rgmID -p rgmpass
				
				""",
				epilog="version {}, copyright {}".format(__version__, __copyright__))
		argparser.add_argument('-H', '--hostname', type=str, help='hostname or IP address', required=True)
		argparser.add_argument('-u', '--user', type=str, help='Clearpass API User', required=True)
		argparser.add_argument('-w', '--warning', type=str, nargs='?', help='warning trigger threshold', default='60')
		argparser.add_argument('-c', '--critical', type=str, nargs='?', help='critical trigger threshold', default='120')
		argparser.add_argument('-i', '--clientID', type=str, help='Clearpass API clientID', required=True)
		argparser.add_argument('-p', '--password', type=str, help='Clearpass API password', required=True)
		#argparser.add_argument('-l', '--list', type=str, help='list all services')
		#argparser.add_argument('-v', '--verbose', help='be verbose', action='store_true')
		args = argparser.parse_args()	
				
		rgm_replication_output(args.hostname, args.user, args.clientID, args.password, args.warning, args.critical)
