...
def rgm_failover_output(clearpass_host, client_id, client_name, client_password):
		try:
				...
				if not member_list:
					...
					outtext.append("Global failover parameters status : {status}".format(
...
if __name__ == '__main__':
        ...
        Check cluster wide failover parameters of a Clearpass Appliance.
				...
				python check_clearpass_API_failover.py -H clearpass1 -u user -i clientID -p password
				
				example : python check_clearpass_API_failover.py -H clearpass1 -u rgm -i rgmID -p rgmpass
		...
		rgm_failover_output(args.hostname, args.user, args.clientID, args.password)
