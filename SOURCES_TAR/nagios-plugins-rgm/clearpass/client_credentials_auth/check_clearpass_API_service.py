...
def get_service_status(clearpass_host, clearpass_service, client_id, client_secret):
    try:
        ...
        print(
            'Error calling "get_service_status"... Exception --- Verify service name ! {}'.format(
                e
            )
        )
        sys.exit(3)
...
def get_all_services_status(
    clearpass_host, clearpass_service, client_id, client_secret
):
    try:
        ...
        # Create correct url to request
        request_url = str(
            "https://" + clearpass_host + ":443/api/server/service/" + server_uuid
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        Get service status of "clearpass1" Cleearpass Server --> Have Critical alert if service is stopped or failed !
            python check_clearpass_api.py -H clearpass1 -n service_name -c clientID -s clientSecret
    ...
    parser.add_argument(
        "-n", "--service", type=str, help="Clearpass service name", required=True
    )
    parser.add_argument(
    ...
    get_service_status(args.hostname, args.service, args.clientID, args.clientSecret)
