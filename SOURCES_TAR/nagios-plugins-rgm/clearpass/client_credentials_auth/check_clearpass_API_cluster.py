...
def get_token(clearpass_host, client_id, client_secret):
    try:
        ...
        print(
            'Error calling "get_token"... Exception {} --- Verify login, mdp or clientID !'.format(
                e
            )
        )
        sys.exit(3)
...
def rgm_cluster_output(clearpass_host, client_id, client_secret):
    try:
        ...
        outtext.append(
            "{member_count} member(s) in cluster".format(member_count=len(member_list))
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        Get cluster members informations of "clearpass1" Clearpass Server
            python check_clearpass_API_cluster.py -H clearpass1 -u user -i clientID -s password
        """,
    ...
    rgm_cluster_output(args.hostname, args.clientID, args.clientSecret)
