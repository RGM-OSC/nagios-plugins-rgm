...
def get_filebackupsrv(clearpass_host, access_token):
    try:
        ...
        request_url = str(
            "https://"
            + clearpass_host
            + ":443/api/file-backup-server?filter=%7B%7D&sort=%2Bid&offset=0&limit=25&calculate_count=false"
        )
...
def rgm_filebackupsrv_output(clearpass_host, client_id, client_secret):
    try:
        ...
        if not filebackupsrv_list:
            ...
            outtext.append(
                "{member_count} file backup servers for host {clearpass_host}".format(
                    member_count=len(filebackupsrv_list), clearpass_host=clearpass_host
                )
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
       ...
       """,
        ...
        Check connectivity with the file backup targets of a Clearpass Appliance. 
             python check_clearpass_API_filebackupsrv.py -H clearpass1 -u user -i clientID -s clientSecret
        """,
    ...
    rgm_filebackupsrv_output(args.hostname, args.clientID, args.clientSecret)
