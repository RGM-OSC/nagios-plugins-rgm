...
def get_events(clearpass_host, access_token, clearpass_severity, event_limit):
    try:
        ...
        request_url = str(
            "https://"
            + clearpass_host
            + ":443/api/system-event?filter="
            + url_filter
            + "&sort=%2Bsource&offset=0&limit="
            + event_limit
...
def rgm_event_output(
    ...
    try:
        ...
        event_list = get_events(
            clearpass_host, access_token, clearpass_severity, event_limit
        )
        ...
        outtext.append(
            "{event_count} Events in {severity} state".format(
                event_count=len(event_list), severity=clearpass_severity
            )
        )
        ...
        print(
            "{}: {} {}".format(
                NagiosRetCode[retcode], " ".join(outtext), " ".join(outevents)
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", "--limit", type=str, help="Max events returned", default="25"
    )
    parser.add_argument(
        "-s",
        "--severity",
        type=str,
        help="Clearpass severity filter : WARNING | ERROR | DEFAULT",
        default="DEFAULT",
    )
    parser.add_argument(
