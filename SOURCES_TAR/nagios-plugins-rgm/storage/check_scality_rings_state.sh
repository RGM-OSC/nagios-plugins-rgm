...
OUTPUT="$(curl --max-time 7 -s -u "${USERNAME}:${PASSWORD}" -X GET "http://${HOSTNAME}/api/v0.1/rings/")"
RETURNCODE="$(echo $?)"
...
RINGList="$(echo $OUTPUT | jq -r '._items[] | .id')"
mi_debug "RINGlist: $RINGList"

if [ ! -n "$(echo ${RINGList} | grep -v null)" ];then
  echo "Le hostname $HOSTNAME ne semble pas présenté de rings !!!"
    exit 3
fi
...
for ring in $RINGList; do
  RINGINFO="$(echo $OUTPUT | jq -r --arg ring_id "$ring" '._items[] | select(.id == $ring_id) | "name: \(.name), type: \(.type), state: \(.state | join(";")) ,status: \(.status), diskspace_available: \(.diskspace_available)"')"
  RINGNAME="$(echo "${RINGINFO}" | sed "s: ::g" | cut -d',' -f1 | cut -d':' -f2)"
  RINGTYPE="$(echo "${RINGINFO}" | sed "s: ::g" | cut -d',' -f2 | cut -d':' -f2)"
  RINGSTATES="$(echo "${RINGINFO}" | sed "s: ::g" | cut -d',' -f3 | cut -d':' -f2)"
  RINGSTATUS="$(echo "${RINGINFO}" | sed "s: ::g" | cut -d',' -f4 | cut -d':' -f2)"
  RINGAVAILBYTES="$(echo "${RINGINFO}" | sed "s: ::g" | cut -d',' -f5 | cut -d':' -f2)"
  SPACEAVAILGB="$(expr $RINGAVAILBYTES / 1000000000)"
  ...
  if [ -n "$OUTPUT_MAIN" ]; then
    OUTPUT_MAIN="$OUTPUT_MAIN \\n $RINGNAME ($RINGTYPE): $RINGSTATES ($RINGSTATUS), Available space: $SPACEAVAILGB GB"
  else
    OUTPUT_MAIN="$RINGNAME ($RINGTYPE): $RINGSTATES ($RINGSTATUS), Available space: $SPACEAVAILGB GB"
  fi

  if [ -n "$OUTPUT_PERF" ]; then
    OUTPUT_PERF="$OUTPUT_PERF \\n$RINGNAME=${SPACEAVAILGB}GB;$WARNING;$CRITICAL"
  else
    OUTPUT_PERF="| $RINGNAME=${SPACEAVAILGB}GB;$WARNING;$CRITICAL"
  fi
