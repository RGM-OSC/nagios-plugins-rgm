...
if ( $getinfos ) {
  ...
  if (defined($xml->{'FIRMWARE_INFORMATION'}[0]->{'INDEX_1'}[0]->{'FIRMWARE_VERSION'}[0]->{'VALUE'})) {
        $message .= "($product[3] - SystemROM: $xml->{'FIRMWARE_INFORMATION'}[0]->{'INDEX_1'}[0]->{'FIRMWARE_VERSION'}[0]->{'VALUE'} -$serial_number $xml->{'FIRMWARE_INFORMATION'}[0]->{'INDEX_4'}[0]->{'FIRMWARE_NAME'}[0]->{'VALUE'} FW $xml->{'FIRMWARE_INFORMATION'}[0]->{'INDEX_4'}[0]->{'FIRMWARE_VERSION'}[0]->{'VALUE'}" . "$server_name) " if defined $product[3];
  } else {
        $message .= "($product[3] -".$serial_number."".$server_name.") " if defined $product[3];
  }
}
...
if ( $optcheckdrives) {
  foreach ( sort keys(%drives) ) {
   if(ref($drives{$_})) {
    ...
    if ( ( $drives{$_} !~ m"^(Ok)$|^(n/a)$|^(Spare)$|^(Not Installed)|^(Not Present/Not Installed)$|^(spun down)$"i ) ) {
      $return = "CRITICAL";
      $message .= "$_: ".$drives{$_}{'status'};
      if (defined $drives{$_}{'model'}){
        $message .= " (Drive ModelNumber: " . $drives{$_}{'model'} ."), ";
...
sub parse_reply
{
  ...
  if ( $getinfos ) {
    $product_name = $line if ( $line =~ m/FIELD NAME="Product Name"/ );
