...
if [ ! -d /tmp/tmp-control-wifi-check_4/${HOSTTARGET} ]; then mkdir -p /tmp/tmp-control-wifi-check_4/${HOSTTARGET}; fi
TMPDIR="/tmp/tmp-control-wifi-check_4/${HOSTTARGET}"

snmpwalk -v 2c -c $COMMUNITY $HOSTTARGET -O 0qv .1.3.6.1.4.1.45.7.3.1.2.1.22 | sed -e 's: "$:":g' > $TMPDIR/snmpwalk_out.txt
...
if [ $LOAD -lt $CRITICAL ]; then
	echo "CRITICAL: less than $CRITICAL AP Update Requiered:$LOAD  :$LIST"
	rm -rf ${TMPDIR}
	exit 2
fi
if [ $LOAD -gt $WARNING ]; then
	echo "WARNING: More then $WARNING AP are Update Requiered: $LOAD   :$LIST"
	rm -rf ${TMPDIR}
	exit 1
fi
echo "OK: number of $CONNECTEDAP Update Requiered.  :$LOAD"
rm -rf ${TMPDIR}
exit 0
