...
if (@List_Diff) {
  ...
  print "CRITICAL: There is $count_test_to_check in $xccdf_profile on server audited_server. | pct_compliance=".$pct_success_to_check."%;$warning;0\n";
...
if ( $pct_success_to_check < $warning ) {
  ...
  print "WARNING: There is $count_test_to_check in $xccdf_profile on server audited_server. | pct_compliance=".$pct_success_to_check."%;$warning;0\n";
  $client->call('auth.logout', $session);
  exit $ERRORS{'OK'}; 
  # exit $ERRORS{'WARNING'};
}
