...
my $DebugFilename = '/tmp/tmp-internal/ctmag/nagios_check.debug';
open(my $dfh, '>>', $DebugFilename) or die "Could not open file '$DebugFilename' $!";
...
my $ChainFile = "/tmp/tmp-internal-" . $chain ."/" . $server ."/Last_State.txt";
...
my $datenow = $dtnow->ymd();
my $timenow = $dtnow->hms();
...
print $dfh "Now(GMT): ($datenow) ($timenow); LastCheck(GMT+1): (@LastCheck);EPOCH Last Check: ($epoch_dtlastcheck); EPOCH Now ($epoch_dtnow); Gap in second ($gap)";
...
sub print_usage () {
    print "Usage:
   $PROGNAME -S servername -C Chain -w warning -c critical [-d] [-h]
   $PROGNAME --help
   $PROGNAME --version
";
}
