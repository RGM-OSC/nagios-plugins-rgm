...
my $o_check_type        = "GetSDXHealth";             # Default check is "GetConcurrentSessionsTrend"
my @valid_types         = ("GetSDXHealth");
my $opt_w              = undef;
my $opt_c              = undef;
...
my $health_text_oid = "1.3.6.1.4.1.5951.6.2.1000.2.1.1"; # could not be finer because not understanding internal Net::SNMP issue.
my $health_value_oid = "1.3.6.1.4.1.5951.6.2.1000.2.1.7";
...
sub help {
        print "\nNetscaler SDX health for Nagios\nVersion: $Version\nDate: $Ver_date\n\n";
        print_usage();
        print <<EOT;
...
if ( $o_check_type eq "GetSDXHealth") {
  ...
  for($GlobalIndex = 0; $GlobalIndex < ($#AutoCheckState + 1); $GlobalIndex++) {
      ...
      $final_output = $final_output.sprintf(" %s: %s \n", $AvailableAutoCheck[$GlobalIndex], $AutoCheckState[$GlobalIndex] );
  }
