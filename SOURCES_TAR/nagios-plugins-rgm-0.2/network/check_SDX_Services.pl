...
my %NetscalerStateCode                 = (1=>'CRITICAL',2=>'CRITICAL',3=>'WARNING',5=>'WARNING',7=>'OK',8=>'WARNING');
my %NetscalerStateCodeMessage           = (1=>'vServer down',2=>'Status unknown',3=>'vServer busy',5=>'vServer transitioning to Out of Service',7=>'Virtual Server is up and available',8=>'vServer transitioning to Out of Service - vServer down');
...
my $o_check_type        = "GetXenServicesState";             # Default check is "GetXenServicesState"
my @valid_types         = ("GetXenServicesState");
my $opt_w              = undef;
my $opt_c              = undef;
...
my $nsServiceState_value_oid = ".1.3.6.1.4.1.5951.6.3.2.1.15.1.4";
my $XenServiceList_oid = ".1.3.6.1.4.1.5951.4.1.3.1.1.1";
my $XenServiceListState_oid = ".1.3.6.1.4.1.5951.4.1.3.1.1.5";
...
sub p_version {
        print "./check_SDX_Services.pl version : $Version $Ver_date\n";
}
...
sub help {
        print "\nNetscaler Xen Virtual Service check for Nagios\nVersion: $Version\nDate: $Ver_date\n\n";
        print_usage();
        print <<EOT;
...
-T, --type=GetXenInstancesState (Default)
   Health check for Netscaler Xen Virtual Instance
...
sub check_options {
        ...
        if ( $o_check_type eq 'NOT Applicable in This Check :P' ) {
          if ( ! defined($opt_w) ) {
                  print_usage();
                  exit $ERRORS{"UNKNOWN"};
          }
...
# ============================================================================
# ======================= GET GetXenInstancesState  =======================
# ============================================================================
sub GetXenServicesState {
  ...
  $session->translate(Net::SNMP->TRANSLATE_NONE);
	my $resultat_status 		=  $session->get_table(Baseoid => $XenServiceList_oid);
  ...
  verb ("Status: ".$resultat_status."\n");
 	if (defined($resultat_status)){	
  	foreach my $key ( keys %$resultat_status) 
  	{
       ...
       push (@AvailableServices, $$resultat_status{$key});
    }
   } else {
  ...
  my $resultat_value 		=  $session->get_table(Baseoid => $XenServiceListState_oid);
     	...
     	foreach my $key ( keys %$resultat_value) 
      	{
           ...
           push (@ServiceState, $$resultat_value{$key});
           #push (@ServiceState, "8"); #Debug purpose only
        }
  } else {
    printf("ERROR requested OID not available.\n");
  	exit $ERRORS{"UNKNOWN"};
...
if ( $o_check_type eq "GetXenServicesState") {

  GetXenServicesState();
  
  if ( $#AvailableServices < 0 ) {
      print "CRITICAL: Can't get Service information.\n";
      exit $ERRORS{"CRITICAL"};
  }
  if ( $#ServiceState < 0 ) {
      print "CRITICAL: Can't get Service state information.\n";
      exit $ERRORS{"CRITICAL"};
  }
  
  $final_status = "OK";
    
  for($GlobalIndex = 0; $GlobalIndex < ($#AvailableServices + 1); $GlobalIndex++) {
    #verb("Key: ".$AvailableServices[$GlobalIndex]." value is: ".$ServiceState[$GlobalIndex]."\n"); 
    
    if ( $ServiceState[$GlobalIndex] == 7 ) {
      my $CurCode="7";
      verb("Service:".$AvailableServices[$GlobalIndex]." is in state ".$NetscalerStateCode{$CurCode}." -> ".$NetscalerStateCodeMessage{$CurCode});
      $final_output = $final_output.sprintf("%s(%s) %s\n", $AvailableServices[$GlobalIndex],$NetscalerStateCode{$CurCode}, $NetscalerStateCodeMessage{$CurCode});
    }
   
    if ( $ServiceState[$GlobalIndex] == 1 ) {
      my $CurCode="1";
      verb("Service:".$AvailableServices[$GlobalIndex]." is in state ".$NetscalerStateCode{$CurCode}." -> ".$NetscalerStateCodeMessage{$CurCode});
      $final_output = $final_output.sprintf("%s(%s) %s\n", $AvailableServices[$GlobalIndex],$NetscalerStateCode{$CurCode}, $NetscalerStateCodeMessage{$CurCode});
      $final_status = $NetscalerStateCode{$CurCode};
    }
   
    if ( $ServiceState[$GlobalIndex] == 2 ) {
      my $CurCode="2";
      verb("Service:".$AvailableServices[$GlobalIndex]." is in state ".$NetscalerStateCode{$CurCode}." -> ".$NetscalerStateCodeMessage{$CurCode});
      $final_output = $final_output.sprintf("%s(%s) %s\n", $AvailableServices[$GlobalIndex],$NetscalerStateCode{$CurCode}, $NetscalerStateCodeMessage{$CurCode});
      $final_status = $NetscalerStateCode{$CurCode};
    ...
    if ( $ServiceState[$GlobalIndex] == 3 ) {
      my $CurCode="3";
      verb("Service:".$AvailableServices[$GlobalIndex]." is in state ".$NetscalerStateCode{$CurCode}." -> ".$NetscalerStateCodeMessage{$CurCode});
      $final_output = $final_output.sprintf("%s(%s) %s\n", $AvailableServices[$GlobalIndex],$NetscalerStateCode{$CurCode}, $NetscalerStateCodeMessage{$CurCode});
      if ( $final_status ne "CRITICAL" ) {
        $final_status = $NetscalerStateCode{$CurCode};
      }
    }
    
    if ( $ServiceState[$GlobalIndex] == 5 ) {
      my $CurCode="5";
      verb("Service:".$AvailableServices[$GlobalIndex]." is in state ".$NetscalerStateCode{$CurCode}." -> ".$NetscalerStateCodeMessage{$CurCode});
      $final_output = $final_output.sprintf("%s(%s) %s\n", $AvailableServices[$GlobalIndex],$NetscalerStateCode{$CurCode}, $NetscalerStateCodeMessage{$CurCode});
      if ( $final_status ne "CRITICAL" ) {
  ...
  print sprintf("%s: Xen services status. Click for detail\n%s\n",$final_status, $final_output);
