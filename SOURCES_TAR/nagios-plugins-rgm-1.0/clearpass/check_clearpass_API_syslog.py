...
def get_syslog(clearpass_host, access_token):
		try:
  			...
  			# Create correct url to request
  			request_url = str("https://" + clearpass_host + ":443/api/syslog-target?filter=%7B%7D&sort=%2Bid&offset=0&limit=25&calculate_count=false")
...
def rgm_syslog_output(clearpass_host, client_id, client_name, client_password):
		try:
				...
				if not syslog_list:
					...
					outtext.append("{member_count} syslog targets for host {clearpass_host}".format(
						member_count=len(syslog_list),
					...
					for server in syslog_list:
						...
						if not check_connectivity(server['host_address'], server['server_port']):
							retcode = 2
							server_status = 2
...
if __name__ == '__main__':
        ...
        Check connectivity with the syslog targets of a Clearpass Appliance. 
				...
				python check_clearpass_API_syslog.py -H clearpass1 -u user -i clientID -p password
				
				example : python check_clearpass_API_syslog.py -H clearpass1 -u rgm -i rgmID -p rgmpass
		...
		rgm_syslog_output(args.hostname, args.user, args.clientID, args.password)
