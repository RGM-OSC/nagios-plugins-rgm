...
'''
DESCRIPTION :
  * Nagios plugin used to return cluster member informations from Clearpass Appliance.
  * 
  * 
...
def rgm_cluster_output(clearpass_host, client_id, client_name, client_password):
		try:
				...
				outtext.append("{member_count} member(s) in cluster".format(
					member_count=len(member_list)))

				for server in member_list:
					outservers.append("\n Name : {name} - Management IP : {mgmt_ip}," \
					" Is Master : {is_master}," \
					" Replication Status : {status} - Last Replication : {timestamp}".format(
					name=server['name'],
					mgmt_ip=server['management_ip'],
					is_master=server['is_master'],
					status=server['replication_status'],
					timestamp=server['last_replication_timestamp']))
			...
			print("Error calling \"rgm_cluster_output\"... Exception --- {}".format(e))
			sys.exit(3)
...
if __name__ == '__main__':
		...
		parser = argparse.ArgumentParser(description="""
				Nagios plugin used to return cluster members informations from a Clearpass Appliance.
				""",
				usage="""
        Get cluster members informations of "clearpass1" Clearpass Server

				python check_clearpass_API_cluster.py -H clearpass1 -u user -i clientID -p password
				
				example : python check_clearpass_API_cluster.py -H clearpass1 -u rgm -i rgmID -p rgmpass
		...
		rgm_cluster_output(args.hostname, args.user, args.clientID, args.password)
