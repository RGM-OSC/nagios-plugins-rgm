...
def get_custom_api_url(prometheus_host, container_image, container_label, interval_start):
    ...
    mem_used_query = urllib.parse.quote(
        "container_memory_usage_bytes{" + container_label + "=~\".*" + container_image + ".*\"}"
    )
    # mem_limit_query = urllib.parse.quote(
    #   "container_spec_memory_limit_bytes{image=~\".*" + container_image + ".*\"}")

    swap_used_query = urllib.parse.quote(
        "container_memory_swap{" + container_label + "=~\".*" + container_image + ".*\"}"
    )
    # swap_limit_query = urllib.parse.quote(
    #   "container_spec_memory_swap_limit_bytes{image=~\".*" + container_image + ".*\"}")

    # Create objects to return
    mem_used_url = str(
        prometheus_host + "/api/v1/query_range?query=" + mem_used_query + "&start=" +
        str(interval_start) + "&end=" + str(interval_end) + "&step=" + step
    )
    # mem_limit_url = str(
    #   prometheus_host + "/api/v1/query_range?query=" + mem_limit_query + "&start=" +
    #   str(interval_start) + "&end=" + str(interval_end) + "&step=" + step
    # )

    swap_used_url = str(
        prometheus_host + "/api/v1/query_range?query=" + swap_used_query + "&start=" +
        str(interval_start) + "&end=" + str(interval_end) + "&step=" + step
    )
    # swap_limit_url = str(
    #   prometheus_host + "/api/v1/query_range?query=" + swap_limit_query + "&start=" +
    #   str(interval_start) + "&end=" + str(interval_end) + "&step=" + step
    # )
...
def rgm_memory_output(
    prometheus_host, container_image, container_label, interval_start, warning_threshold,
    critical_threshold, verbose
):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        ...
        * Memory request is handled by API REST againt Prometheus.
        ...
        Critical and Warning threshold alerts are implemented in MB threshold and not percent from lack of max memory
        values
        """,
    ...
    parser.add_argument(
        '-w', '--warning', type=str, nargs='?',
        help='warning trigger threshold (physical,swap)', default='1024,256'
    )
    parser.add_argument(
        '-c', '--critical', type=str, nargs='?',
        help='critical trigger threshold (physical,swap)', default='2048,512'
    )
    parser.add_argument(
        '-i', '--intervalstart', type=int,
        help='timestamp value in seconds to start the memory usage check - default to 1min',
