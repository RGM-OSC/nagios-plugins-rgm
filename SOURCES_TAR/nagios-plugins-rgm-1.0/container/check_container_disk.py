...
def get_custom_api_url(prometheus_host, container_name, interval_start):
    ...
    disk_used_query = urllib.parse.quote("container_fs_usage_bytes{name=~\"" + container_name + "\"}")
    disk_limit_query = urllib.parse.quote("container_fs_limit_bytes{name=~\"" + container_name + "\"}")

    # Create objects to return
    disk_used_url = "{host}/api/v1/query_range?query={disk}&start={start}&end={end}&step={step}".format(
        host=prometheus_host,
        disk=disk_used_query,
        start=interval_start,
        end=interval_end,
        step=step
    )
    disk_limit_url = "{host}/api/v1/query_range?query={disk}&start={start}&end={end}&step={step}".format(
        host=prometheus_host,
        disk=disk_limit_query,
        start=interval_start,
        end=interval_end,
        step=step
    )
...
def convert_bytes(prometheus_host, container_name, interval_start, warning_threshold, critical_threshold, verbose):
    try:
        ...
        if results['disk_limit'] == -1:
            raise Exception('No disk limit found.')
        elif results['disk_used'] == -1:
            raise Exception('No disk usage found.')
        else:
...
def rgm_disk_output(prometheus_host, container_name, interval_start, warning_threshold, critical_threshold, verbose):
    ...
    try:
        ...
        results = convert_bytes(
            prometheus_host, container_name, interval_start, warning_threshold, critical_threshold, verbose
        )
        ...
        if nagios_status != 0:
            ...
            if len(crit) > 0:
                outtext.append(
                    "{} devices in CRITICAL state ({}%): {}".format(
                        str(len(crit)),
            ...
            if len(warn) > 0:
                outtext.append(
                    "{} devices in WARNING state ({}%): {}".format(
                        str(len(warn)),
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help='timestamp value in seconds to start the disk usage check - default to 6min',
