...
def get_custom_api_url(prometheus_host, container_image, container_label, interval_start):
	...
	prometheus_query = urllib.quote("sum(rate(container_cpu_usage_seconds_total{" + container_label + "=~\".*" + container_image + ".*\"}[" + step + "s]))*100")

	# Create object to return
	request_url = str(prometheus_host + "/api/v1/query_range?query=" + prometheus_query + "&start=" + str(interval_start) + "&end=" + str(interval_end) + "&step=" + step)
...
def rgm_cpu_output(prometheus_host, container_image, container_label, interval_start, warning_threshold, critical_threshold, verbose):
...
if __name__ == '__main__':
	...
	* CPU request is handled by API REST againt Prometheus.
	...
	parser.add_argument('-i', '--intervalstart', type=int, help='timestamp value in seconds to start the CPU usage check - default to 6min', default=int(time.time())-60)
	...
	rgm_cpu_output(args.prometheushost, args.hostname, args.label, args.intervalstart, args.warning, args.critical, args.verbose)
